...
#define MARS_LCD1602_CSI0_DAT4__GPIO_5_22_PIN               IMX_GPIO_NR(5,22)   // LCD1602_D4   pin25
#define MARS_LCD1602_CSI0_DAT5__GPIO_5_23_PIN               IMX_GPIO_NR(5,23)   // LCD1602_D5   pin27
#define MARS_LCD1602_CSI0_DAT6__GPIO_5_24_PIN               IMX_GPIO_NR(5,24)   // LCD1602_D6   pin29
#define MARS_LCD1602_CSI0_DAT7__GPIO_5_25_PIN               IMX_GPIO_NR(5,25)   // LCD1602_D7   pin31
...
static void LcdWriteCommand(int cmd)
{
    int temp;
    
    Config_Mars_Lcd1602_Gpio_Level(MARS_LCD1602_SD4_DAT3__GPIO_2_11_PIN, 0);        // RS = LOW, command
    Config_Mars_Lcd1602_Gpio_Level(MARS_LCD1602_GPIO_18__GPIO_7_13_PIN, 0);         // RW = LOW, write
    Config_Mars_Lcd1602_Gpio_Level(MARS_LCD1602_GPIO_17__GPIO_7_12_PIN, 0);         // Enable to LOW

    temp = cmd & 0xf0;

    Config_Mars_Lcd1602_Gpio_Level(MARS_LCD1602_CSI0_DAT4__GPIO_5_22_PIN, temp & 0x80);
    temp <<= 1;
    
    Config_Mars_Lcd1602_Gpio_Level(MARS_LCD1602_CSI0_DAT5__GPIO_5_23_PIN, temp & 0x80);
    temp <<= 1;

    Config_Mars_Lcd1602_Gpio_Level(MARS_LCD1602_CSI0_DAT6__GPIO_5_24_PIN, temp & 0x80);
    temp <<= 1;

    Config_Mars_Lcd1602_Gpio_Level(MARS_LCD1602_CSI0_DAT7__GPIO_5_25_PIN, temp & 0x80);
    temp <<= 1;
...
static void LcdWriteData(int dat)
{
    int temp;
    
    Config_Mars_Lcd1602_Gpio_Level(MARS_LCD1602_SD4_DAT3__GPIO_2_11_PIN, 1);        // RS = HIGH, data
    Config_Mars_Lcd1602_Gpio_Level(MARS_LCD1602_GPIO_18__GPIO_7_13_PIN, 0);         // RW = LOW, write
    Config_Mars_Lcd1602_Gpio_Level(MARS_LCD1602_GPIO_17__GPIO_7_12_PIN, 0);         // Enable to LOW
...
static int mars_lcd1602_open(struct inode *inode, struct file *filp)
{
    ...
    LcdWriteCommand(0x28);  // 设置为 4-bit接口，2行显示，5x7文字大小
    mdelay(50);
